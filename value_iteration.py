...
def action(s, a, u):
    outcomes = get_outcome_states(s, a)
    res = 0
    for o in outcomes:
        res += get_transition_probability(s, a, o) * u[o]
    return res

def maximum_action(s, u):
    actions = []
    for a in moves:
        actions.append(action(s, a, u))
    return max(actions)
...
def value_iteration() -> Any:
    ...
    """
    u1 = [0]*constants.number_states # utilities of states, initially zero
    u2 = [0]*constants.number_states # utilities of the next states, initially zero
    delta = 10 # the maximum change in the utility of any state in an iteration

    while delta >= constants.epsilon*(1-constants.gamma)/constants.gamma:
        u1 = u2.copy()
        delta = 0
        for s in range(constants.number_states):
            u2[s] = get_reward(s) + constants.gamma * maximum_action(s, u1)
            if (u2[s]-u1[s]) > delta:
                delta = u2[s]-u1[s]
    return [round(num, 2) for num in u1]

def display(value_table):
    column_names = ["1", "2", "3", "4"]
    row_names = ["1", "2", "3", "4"]
    matrix = np.reshape(np.array(value_table), (4,4))
    df = pd.DataFrame(matrix, columns=column_names, index=row_names)
    print(df)

def optimal_policy(s, value_table):
    best_value = 0
    best_move = ""

    for a in moves:
        value = action(s, a, value_table)
        if value > best_value:
            best_value = value
            best_move = a
    return best_move
...
def extract_policy(value_table: Any) -> Any:
    ...
    """
    op = [""]*constants.number_states
    for s in range(constants.number_states):
        op[s] = optimal_policy(s, value_table)
        op[-1] = "goal" # because the last one is the goal
    return op
